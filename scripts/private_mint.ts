...
async function private_mint() {
    ...
    for (let row of rows) {
        ...
        list_to_mint.push( {address : data_row[0], ring_type : data_row[1]} );
    }
    ...
    const runiverseMinterContract = await ethers.getContractAt("WatchersRingMinter", runiverseMinterContractAddress);

    //This is hardhat address, change when deployed to another network.
    const runiverseContractAddress = "0x5FbDB2315678afecb367f032d93F642f64180aa3";
    const runiverseContract = await ethers.getContractAt("WatchersRing", runiverseContractAddress);
    ...
    for(let r =  from_id ; r <=  to_id; r+=BATCH_SIZE ){        
        ...
        for(let c = r; c< Math.min(to_id + 1, r + BATCH_SIZE); c++ ){
            ...
            console.log('Batch mint', c, list_to_mint[c].address, list_to_mint[c].ring_type);
        }
        console.log("Waiting for tx");
        const tx = await runiverseMinterContract.ownerMint(ringTypes, addresses, {gasLimit: 8000000, gasPrice:gasPrice })
